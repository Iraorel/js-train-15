...
function findMinMax(numbers) {
  ...
  if(!Array.isArray(numbers)) {
    console.error("Помилка: переданий параметр має бути масивом чисел.");
   
    return null;
  }
  if (numbers.length === 0) {
    console.error("Помилка: масив чисел порожній.");
    return null;
  }
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== 'number' || typeof height !== 'number') {
    ...
    console.error("Помилка: обидва аргументи повинні бути числами");
...
function getFractionalPart(num) {
  ...
  const fractionalPart = num - roundedDown;
